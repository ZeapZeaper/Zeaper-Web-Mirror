...
const productGroupNavOptions = [
  {
    label: "HOME",
    href: "/",
    imageLink: "/collections/gender=Female/ageGroup=Adults",
    params: { gender: "Female", ageGroup: "Adults" },
    image: HomeImage,
  ...
  },
  {
    ...
    image: AccessImage,
    linkRoot: "/products/collections/Accessory",
...
const WomenChildrenSubMenus = ({
  ...
  const WomenProductsDynamicFiltersQuery =
  ...
  const filteredProductsDynamicFilters = useMemo(() => {
    const productsDynamicFilters =
      WomenProductsDynamicFiltersQuery?.data?.data || [];
              ...
              .map((option) => ({
                ...
                collectionTitle: `WOMEN'S ${
                  found?.baseCollectionTitle
                    ? `${found.baseCollectionTitle} - `
                    : ""
                } ${formatName(filter.name)} - ${option.value}`,
              }))
  ...
  const getImageObj = () => {
    ...
    if (pathname === "/bespoke" || productGroupPage === "BESPOKE") {
      imgaeObj.label = "Shop All Bespoke Women";
    }
    if (pathname === "/ready-to-wear" || productGroupPage === "READY TO WEAR") {
      imgaeObj.label = "Shop All Women Ready to Wear";
    }
    if (pathname === "/accessories" || productGroupPage === "ACCESSORIES") {
      imgaeObj.label = "Shop All Women Accessories";
    }
    return imgaeObj;
  };
  return (
...
export default WomenChildrenSubMenus;
