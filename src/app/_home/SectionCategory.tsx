    ...
    <div className="grid grid-cols-2  md:grid-cols-4 gap-4 ">
      {slideCategories.slice(0, 4).map((item) => (
        <Link
          key={item.title}
          href={`${item.link}`}
          className="relative mt-[10rem] sm:mt-[12rem] md:mt-[5rem]  flex h-[100px] md:h-[80px] lg:h-[120px] 2xl:h-[240px] w-full items-center justify-center rounded-2xl bg-secondary hover:bg-lightGold transition-all duration-300 ease-in-out cursor-pointer"
        >
          <Image
            src={item.coverImage}
            alt={item.title}
