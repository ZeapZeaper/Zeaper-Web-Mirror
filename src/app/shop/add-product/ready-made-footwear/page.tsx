...
const AddReadyMadeShoePage = () => {
  ...
  const styleOptions = options?.readyMadeShoes?.shoeStyleEnums
    ?.map((str: string, index: number) => ({ value: str, id: index + 1 }))
    .sort((a: OptionType, b: OptionType) => a.value.localeCompare(b.value));
  const genderOptions = options?.readyMadeShoes?.genderEnums
    ?.map((str: string, index: number) => ({ value: str, id: index + 1 }))
    .sort((a: OptionType, b: OptionType) => a.value.localeCompare(b.value));
  ...
  const designOptionEnums = options?.readyMadeShoes?.designEnums
    ?.map((str: string, index: number) => ({ value: str, id: index + 1 }))
    .sort((a: OptionType, b: OptionType) => a.value.localeCompare(b.value));
  const occasionOptionEnums = options?.readyMadeShoes?.occasionEnums
    ?.map((str: string, index: number) => ({ value: str, id: index + 1 }))
    .sort((a: OptionType, b: OptionType) => a.value.localeCompare(b.value));
...
export default AddReadyMadeShoePage;
