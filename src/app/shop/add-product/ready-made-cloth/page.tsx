...
const AddReadyMadeClothPage = () => {
  ...
  const mainOptions = options?.readyMadeClothes?.mainEnums
  ...
  const designOptionEnums = options?.readyMadeClothes?.designEnums
    ?.map((str: string, index: number) => ({ value: str, id: index + 1 }))
    .sort((a: OptionType, b: OptionType) => a.value.localeCompare(b.value));
  const occasionOptionEnums = options?.readyMadeClothes?.occasionEnums
    ?.map((str: string, index: number) => ({ value: str, id: index + 1 }))
    .sort((a: OptionType, b: OptionType) => a.value.localeCompare(b.value));
  const sleeveLengthEnums = options?.readyMadeClothes?.sleeveLengthEnums;
  const colorEnums = options?.readyMadeClothes?.colorEnums;
  const fasteningOptionEnums = options?.readyMadeClothes?.fasteningEnums
    ?.map((str: string, index: number) => ({ value: str, id: index + 1 }))
    .sort((a: OptionType, b: OptionType) => a.value.localeCompare(b.value));
...
export default AddReadyMadeClothPage;
